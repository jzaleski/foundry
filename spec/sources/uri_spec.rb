...
describe Foundry::Sources::URI do
  ...
  let(:relative_uri) { 'config.yml' }

  let(:http_uri) { "#{http_config_root}/#{relative_uri}" }
  let(:http_config_root) { "http://#{domain}" }
  let(:http_response_body) { 'http_response_body' }
  let(:http_opts) { {} }

  let(:https_uri) { "#{https_config_root}/#{relative_uri}" }
  let(:https_config_root) { "https://#{domain}" }
  let(:https_response_body) { 'https_response_body' }
  let(:https_opts) { {} }

  let(:basic_auth_uri) { "http://#{basic_auth_username}:#{basic_auth_password}@#{domain}/#{relative_uri}" }
